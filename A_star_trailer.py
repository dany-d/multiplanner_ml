    ...
    for x in range(2):
        distance += (((config1.position[0] - config2.position[0]) ** 2) +
                     ...
                     10*(config1.position[2] - config2.position[2]) ** 2 +
                      (config1.position[3] - config2.position[3]) ** 2
                    + AngDiff(config1.position[2], config2.position[2])**2
                    + AngDiff(config1.position[3], config2.position[3])**2)
                     #
                     # +(config1.position[2] - config2.position[2] + 2*numpy.pi) ** 2
                     # +(config1.position[2] - config2.position[2] - 2*numpy.pi) ** 2
                     # + (config1.position[3] - config2.position[3] + 2 * numpy.pi) ** 2
                     # + (config1.position[3] - config2.position[3] - 2 * numpy.pi) ** 2)
...
def astar(start, end,weight=2):
    ...
    while len(open_list) > 0:
        ...
        step = 10
        # Found the goal
        if euclideanDistance(current_node, end_node) < 10:
            path = []
        ...
        # Generate children
        children = []
        new_nodes = steering_fun(current_node.position[0],current_node.position[1],current_node.position[2],current_node.position[3])
        for node_position in new_nodes:  # Adjacent squares
            ...
            # Check for collision!!
            if collision_check(node_position[0], node_position[1], node_position[2],trailerTheta=node_position[3]) != 0:
...
def main():
    start_q, goal_q = init_collision_checking("Automap_data/autoMaps/AG_X_249.png")
    start_q = (start_q[0],start_q[1],start_q[2],start_q[2])
    goal_q = (goal_q[0],goal_q[1],goal_q[2],goal_q[2])
