...
import numpy
import time
from angle_calc import angle, AngDiff
...
def dist(point1, point2):
    diff_angle=AngDiff(point1[2],point2[2])
    return sqrt((point1[0] - point2[0]) ** 2 + (point1[1] - point2[1])** 2 + 10*(diff_angle)** 2)
...
def smooth(path,n):
    ...
    for k in range(n):
        ...
        while dist(node1,node2)>step:
            mid_node=steer_smooth(node1,node2)
            if collision_check(mid_node[0], mid_node[1], mid_node[2]):
                local_flag=0
                break
            else:
...
def steering_fun(x,y,th,z_randA):
    curve=5
    ...
    nodes = []
    for i in phi:
        dx = v_step*cos(i)*cos(th)
        dy = v_step*cos(i)*sin(th)
        dth = v_step/L*sin(i)
        nodes.append((x+dx,y+dy,th+dth))

    # Need to update the euclidean distance
    # the angle between two configurations should be the shortest angle
    # use angle function for that
    nodes.sort(key=lambda x: dist(x, z_randA))

    for i in range(len(nodes)):
        closest = nodes[i]
        if collision_check(closest[0],closest[1], closest[2]):
            continue
        else:
            return Point(closest[0],closest[1], closest[2])

def neigh_closest(nodes, point):
    return

def birrt(start, Goal):
    T0 = time.time()
    start_pos = Point(start[0], start[1], start[2])
    end_pos = Point(Goal[0], Goal[1], Goal[2])
    ...
    while True:
        ...
        while i<=j: # To Ensure Both Trees are Balanced
            if time.time() - T0 > 15:  # fifteen secs
                return None
            z_randA = Point(randint(0, WORLD_SIZE), randint(0, WORLD_SIZE),uniform(-numpy.pi,numpy.pi))

            if collision_check(z_randA[0], z_randA[1],z_randA[2]):
                continue
            ...
            new_posA=steering_fun(nearestA.pos[0], nearestA.pos[1],nearestA.pos[2],z_randA)
            if new_posA is None:
                continue
            ...
            while dist(new_posA,nearestinB.pos)>step:
                newer_posA = steering_fun(new_posA[0], new_posA[1],new_posA[2],nearestinB.pos)
                ...
                if dist(nearestinB.pos,newer_posA)<=step:
            ...
            if flag==1:
                ...
                if current_node_g.id == 0:
                    path.append([current_node_g.pos.x, current_node_g.pos.y, current_node_g.pos.th])
                    set_goal=0
                ...
                while current_node_s.id != 0:
                    ...
                    path.append([current_node_s.pos.x, current_node_s.pos.y, current_node_s.pos.th])
                    current_node_s = parent

                path.append([current_node_s.pos.x, current_node_s.pos.y, current_node_s.pos.th]) # Add start to the que
        ...
        while j <= i:
            if time.time() - T0 > 15:  # fifteen secs
                return None
            z_randB = Point(randint(0, WORLD_SIZE), randint(0, WORLD_SIZE),uniform(-numpy.pi,numpy.pi))

            if collision_check(z_randB[0], z_randB[1],z_randB[2]):
                continue
            ...
            new_posB = steering_fun(nearestB.pos[0], nearestB.pos[1], nearestB.pos[2], z_randB)
            if new_posB is None:
                continue
            ...
            while dist(new_posB, nearestinA.pos) > step:
                newer_posB = steering_fun(new_posB[0], new_posB[1],new_posB[2],nearestinA.pos)
                ...
                if dist(nearestinA.pos, newer_posB) <= step:
...
if __name__ == '__main__':
    start, Goal=init_collision_checking("Automap_data/autoMaps/AG_S_146.png")
