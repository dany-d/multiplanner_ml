...
from A_star_trailer_2 import run as Astar2Runner
...
def save_map(basemap_path):
    ...
    # plt.imshow(img)
    plt.imsave("autoMaps_trailer/1/AG_"+map_type+"_"+str(save_count), im_arr)
    save_count += 1
    ...
    for base_map_path, map_count in path_and_count:
        ...
        while map_num < map_count:
            ...
            while not start_valid:
                ...
                # check if its valid
                theta = rand_theta()
                if not collision_check(x,y,theta,trailerTheta=theta):
                    start_valid = True
                    # place start
            ...
            while not goal_valid:
                ...
                if ( (not collision_check(x,y,theta,trailerTheta=theta)) and (check_dist(x,y,start_xy[0],start_xy[1]) > 40)) :
                    goal_valid = True
                    # place goal
