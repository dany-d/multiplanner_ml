    ...
    while True:
        ...
        while i<=j: # To Ensure Both Trees are Balanced
            ...
            while dist(new_posA,nearestinB.pos)>step:
                newer_posA = steer(new_posA,nearestinB.pos)
                if collision_check(newer_posA[0], newer_posA[1], newer_posA[2]):
                    break
                else:
        ...
        while j <= i:
            ...
            while dist(new_posB, nearestinA.pos) > step:
                newer_posB = steer(new_posB,nearestinA.pos)
                if collision_check(newer_posB[0], newer_posB[1], newer_posB[2]):
                    break
                else:
...
if __name__ == '__main__':
    start, Goal=init_collision_checking("Automap_data/autoMaps/AG_X_237.png")
